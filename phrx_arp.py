...
def _phrx_arp(self, packet_rx):
    ...
    if packet_rx.arp.oper == ps_arp.ARP_OP_REQUEST:
        ...
        if packet_rx.arp.spa in self.ip4_unicast:
            self.logger.warning(f"IP ({packet_rx.arp.spa}) conflict detected with host at {packet_rx.arp.sha}")
            return
    ...
    # Handle ARP reply
    elif packet_rx.arp.oper == ps_arp.ARP_OP_REPLY:
