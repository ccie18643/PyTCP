...
class TestUdpPhtx(TestCase):
    ...
    def test_tcp_phtx__ip4_tcp_packet__no_data(self):
        """Test sending IPv4/TCP packet with no data"""
        ...
        with open(TEST_FRAME_DIR + "ip4_tcp_packet__no_data.tx", "rb") as _:
            frame_tx = _.read()
        self.assertEqual(self.frame_tx[: len(frame_tx)], frame_tx)

    def test_udp_phtx__ip4_tcp_packet__with_data(self):
        ...
        with open(TEST_FRAME_DIR + "ip4_tcp_packet__with_data.tx", "rb") as _:
            frame_tx = _.read()
        self.assertEqual(self.frame_tx[: len(frame_tx)], frame_tx)
