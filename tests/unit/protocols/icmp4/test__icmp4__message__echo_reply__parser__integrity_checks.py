...
tests/unit/protocols/icmp4/test__icmp4__message__echo_reply__parser__integrity_checks.py
...
@parameterized_class(
    [
        {
            "_description": (
                "ICMP4 Echo Reply message, the 'ICMP4_HEADER_LEN <= self._ip4_payload_len' "
                "condition not met."
            ),
            "_args": {
                "bytes": b"\x00\x00\xfb",
        ...
        {
            "_description": (
                "ICMP4 Echo Reply message, the 'self._ip4_payload_len <= len(self._frame)' "
                "condition not met."
            ),
            "_args": {
        ...
        {
            "_description": (
                "ICMP4 Echo Reply message, the 'ICMP4_ECHO_REPLY_LEN <= self._ip4_payload_len' "
                "condition not met."
            ),
            "_args": {
                "bytes": b"\x00\x00\xfb\x94\x30\x39\xd4",
        ...
        {
            "_description": "ICMP4 Echo Reply message, invalid checksum.",
            "_args": {
...
class TestIcmp4EchoReplyMessageParserIntegrityChecks(TestCase):
    """
    ...
    def test__icmp4__message__echo_reply__parser__from_bytes(self) -> None:
        """
        Ensure the ICMPv4 Echo Reply message parser raises integrity error
