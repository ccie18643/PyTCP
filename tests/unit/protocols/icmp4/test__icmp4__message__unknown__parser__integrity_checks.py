...
tests/unit/protocols/icmp4/test__icmp4__message__unknown__parser__integrity_checks.py
...
@parameterized_class(
    [
        {
            "_description": (
                "ICMP4 Unknown message, the 'ICMP4_HEADER_LEN <= self._ip4_payload_len' "
                "condition not met."
            ),
            "_args": {
                "bytes": b"\xff\x00\xfb",
        ...
        {
            "_description": (
                "ICMP4 unknown message, the 'self._ip4_payload_len <= len(self._frame)' "
                "condition not met."
            ),
            "_args": {
                "bytes": b"\xff\x00\xfb\x94\x30\x39\xd4",
            },
            "_conditions": {
                "ip4__payload_len": 8,
            },
        ...
        {
            "_description": "ICMP4 unknown message, invalid checksum.",
            "_args": {
                "bytes": b"\xff\x00\x00\x00\x30\x39\xd4\x31",
            },
            "_conditions": {},
            "_results": {
...
class TestIcmp4UnknwonwMessageParserIntegrityChecks(TestCase):
    """
    ...
    def test__icmp4__message__unknown__parser__from_bytes(self) -> None:
        """
        Ensure the ICMPv4 unknown message parser raises integrity error
