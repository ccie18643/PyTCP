...
class PacketHandler:
    ...
    def __init__(self):
        ...
        if config.ip6_support:
            ...
            stack.ip6_address_candidate = self.parse_stack_ip6_address_candidate(config.ip6_address_candidate)
        ...
        if config.ip4_support:
            ...
            stack.ip4_address_candidate = self.parse_stack_ip4_address_candidate(config.ip4_address_candidate + ip4_address_dhcp)
            self.create_stack_ip4_addressing()

        # Log all the addresses stack will listen on
        self.logger.info(f"Stack listening on unicast MAC address: {stack.mac_unicast}")
        self.logger.info(f"Stack listening on multicast MAC addresses: {list(set(stack.mac_multicast))}")
        self.logger.info(f"Stack listening on brodcast MAC address: {stack.mac_broadcast}")

        if config.ip6_support:
            self.logger.info(f"Stack listening on unicast IPv6 addresses: {[str(_) for _ in stack.ip6_unicast]}")
            self.logger.info(f"Stack listening on multicast IPv6 addresses: {list(set(str(_) for _ in stack.ip6_multicast))}")

        if config.ip4_support:
            self.logger.info(f"Stack listening on unicast IPv4 addresses: {[str(_) for _ in stack.ip4_unicast]}")
            self.logger.info(f"Stack listening on multicast IPv4 addresses: {[str(_) for _ in stack.ip4_multicast]}")
            self.logger.info(f"Stack listening on brodcast IPv4 addresses: {[str(_) for _ in stack.ip4_broadcast]}")
    ...
    def create_stack_ip6_addressing(self):
        ...
        for ip6_address in list(stack.ip6_address_candidate):
            if ip6_address.ip.is_link_local:
                stack.ip6_address_candidate.remove(ip6_address)
                __(ip6_address)

        # Configure Link Local address automaticaly
        if config.ip6_lla_autoconfig:
            ip6_address = IPv6Network("fe80::/64").eui64(stack.mac_unicast)
            ip6_address.gateway = None
            __(ip6_address)

        # If we don't have any link local address set disable IPv6 protocol operations
        if not stack.ip6_address:
        ...
        if config.ip6_gua_autoconfig:
            ...
            for prefix, gateway in list(self.icmp6_ra_prefixes):
                ...
                ip6_address = prefix.eui64(stack.mac_unicast)
    ...
    def create_stack_ip4_addressing(self):
        ...
        for _ in range(3):
            for ip4_unicast in [_.ip for _ in stack.ip4_address_candidate]:
        ...
        for ip4_address in list(stack.ip4_address_candidate):
            stack.ip4_address_candidate.remove(ip4_address)
            if ip4_address.ip not in self.arp_probe_unicast_conflict:
                stack.ip4_address.append(ip4_address)
    ...
    def send_icmp6_multicast_listener_report(self):
        ...
        if icmp6_mlr2_multicast_address_record := {
            ...
            for _ in stack.ip6_multicast
            if _ not in {IPv6Address("ff02::1")}
        }:
            self.phtx_icmp6(
                ip6_src=stack.ip6_unicast[0] if stack.ip6_unicast else IPv6Address("::"),
    ...
    def remove_ip6_multicast(self, ip6_multicast):
        ...
        stack.ip6_multicast.remove(ip6_multicast)
    ...
    def assign_mac_multicast(self, mac_multicast):
        ...
        stack.mac_multicast.append(mac_multicast)
    ...
    def remove_mac_multicast(self, mac_multicast):
        ...
        stack.mac_multicast.remove(mac_multicast)
