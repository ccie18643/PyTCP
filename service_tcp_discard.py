...
class ServiceTcpDiscard:
    ...
    def __thread_service(self, local_ipv4_address, local_port):
        ...
        socket.listen()
        print(f"Service TCP Discard: Socket created, bound to {local_ipv4_address}:{local_port} and set to listening mode")

        while True:
            new_socket = socket.accept()
            print(f"Service TCP Discard: Inbound connection received from {new_socket.remote_ipv4_address}:{new_socket.remote_port}")
    ...
    def __thread_connection(self, socket):
        ...
        while True:
            ...
            print(f"Service TCP Discard: Discarded message from {socket.remote_ipv4_address}:{socket.remote_port} -", message)

        socket.close()
        print(f"Service TCP Discard: Connection from {socket.remote_ipv4_address}:{socket.remote_port} has been closed by peer")
