...
| 7 **LANDED** | `ip6_ifaddr.py` `from_rfc7217` | Added a byte-exact golden vector (full prefix + MAC + secret + dad → known IID) pinning the whole PRF construction (F() input assembly, digest slice, prefix/IID combine) the differential tests left free, plus an exactly-16-byte secret happy case. Kills the L269/271/277 hash arithmetic, L281 netmask, and L260 (`<`→`<=`) survivors. |
| 8 | `ip6_ifaddr.py` `_is_reserved_iid` | The 2 survivors (L74/76) gate the RFC 5453 reserved-IID regeneration; killing them deterministically needs an input that hashes *into* the reserved range — not constructible without grinding the secret. Deferred (effectively equivalent for a non-grinding test). |
| 9 **LANDED** | `mac_address.py` | Most survivors are equivalent: a single-bit `&` result is 0-or-bit so `==`≡`<=`/`>=` (L151/159), broadcast is the max value so `==`≡`>=` (L191), and `_format_alt` L133 `range(0,12,4)`→`range(0,11,4)` yields identical indices. The one real fixture gap (L42, the I/G-bit constant masked because every tested multicast MAC ends in an odd octet) is closed with a multicast MAC ending in an even octet. |
| 10 **LANDED** | `ip_wildcard.py` / `ip*_wildcard.py` | The `__or__` version checks (L91/94) are equivalent (redundant after the `isinstance` guard — always `IP4==IP4`). The real survivors are `__eq__` L113 (`==`→`>=`, only one inequality direction tested) and `__init__` L79 (`==`→`>=` bytes-length, only too-short bytes tested). Closed with a larger-wildcard-on-the-left inequality and a too-long-bytes rejection in both families. |
