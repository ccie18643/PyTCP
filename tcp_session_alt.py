...
class TcpSession:
    ...
    def _transmit_packet(self, seq=None, flag_syn=False, flag_ack=False, flag_fin=False, flag_rst=False, raw_data=b""):
        """ Send out TCP packet """
    ...
    def _enqueue_rx_buffer(self, raw_data):
        ...
        if self.state in {"ESTABLISHED", "CLOSE_WAIT"}:
            ...
            if remaining_data_len:
                ...
                if transmit_data_len:
                    ...
                    self._transmit_packet(flag_ack=True, raw_data=bytes(transmit_data))
                return
    ...
    def _process_ack_packet(self, packet):
        ...
        if packet.raw_data:
            self._enqueue_rx_buffer(packet.raw_data)
            if __debug__:
    ...
    def _tcp_fsm_established(self, packet, syscall, timer):
        ...
        if packet and not self.rcv_nxt <= packet.seq <= self.rcv_nxt + self.rcv_wnd - len(packet.raw_data):
            if __debug__:
                self._logger.debug(f"{self.tcp_session_id} - Packet seq {packet.seq} + {len(packet.raw_data)} doesn't fit into receive window, dropping")
            return
