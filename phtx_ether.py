...
def phtx_ether(self, child_packet, ether_src="00:00:00:00:00:00", ether_dst="00:00:00:00:00:00"):
    ...
    if ether_packet_tx.ether_type == ps_ether.ETHER_TYPE_IP6:
        ...
        if ip6_packet_tx.ip6_dst.is_multicast:
            ...
            if __debug__:
                self._logger.debug(f"{ether_packet_tx.tracker} - Resolved destination IPv6 {ip6_packet_tx.ip6_dst} to MAC {ether_packet_tx.ether_dst}")
            __send_out_packet()
            return
    ...
    if ether_packet_tx.ether_type == ps_ether.ETHER_TYPE_IP4:
        ...
        if ip4_packet_tx.ip4_dst.is_limited_broadcast:
            ...
            if __debug__:
                self._logger.debug(f"{ether_packet_tx.tracker} - Resolved destination IPv4 {ip4_packet_tx.ip4_dst} to MAC {ether_packet_tx.ether_dst}")
            __send_out_packet()
            return
