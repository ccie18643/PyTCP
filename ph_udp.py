...
class UdpPacket:
    ...
    def validate_cksum(self):
        ...
        cksum_data = list(struct.unpack(f"! {(len(self.ip_pseudo_header) + (self.hdr_len + 1 if self.hdr_len & 1 else self.hdr_len)) >> 1}H",
    ...
    def dump(self):
        ...
        return (
            "--------------------------------------------------------------------------------\n"
            + f"UDP      SPORT {self.hdr_sport}  DPORT {self.hdr_dport}  LEN {self.hdr_len}  CKSUM {self.hdr_cksum} ({'OK' if self.validate_cksum() else 'BAD'})"
        )
...
class UdpPacketRx(UdpPacket):
    ...
    def __init__(self, raw_packet, ip_pseudo_header):
        """ Class constructor """
...
class UdpPacketTx(UdpPacket):
    ...
    def raw_header(self):
        ...
        return struct.pack("! HH HH", self.hdr_sport, self.hdr_dport, self.hdr_len, sefl.hdr_cksum)
