...
class TcpOptionWscale(TcpOption):
    ...
    def __post_init__(self) -> None:
        ...
        assert (
            ...
            f"The 'wscale' field must be a 8-bit unsigned integer less than or equal "
            f"to {TCP__OPTION_WSCALE__MAX_VALUE}. Got: {self.wscale}"
        )
    ...
    def from_bytes(_bytes: bytes) -> TcpOptionWscale:
        ...
        assert len(_bytes) >= 2
        assert _bytes[0] == int(TcpOptionType.WSCALE)

        if _bytes[1] != TCP__OPTION_WSCALE__LEN:
            raise TcpIntegrityError("Invalid Wscale option length (I).")

        if _bytes[1] > len(_bytes):
            raise TcpIntegrityError("Invalid Wscale option length (II).")
