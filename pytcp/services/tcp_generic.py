...
class ServiceTcp:
    ...
    def __thread_service(self) -> None:
        ...
        if version == 6:
            ...
            if __debug__:
                log(
                    "service",
                    f"Service TCP {self.name}: Invalid local IP address - "
        ...
        try:
            ...
            if __debug__:
                log(
                    "service",
                    f"Service TCP {self.name}: Socket created, bound to "
            ...
            if __debug__:
                log(
                    "service",
                    f"Service TCP {self.name}: bind() call failed - {error}",
                )
            return
        ...
        if __debug__:
            log(
                "service",
                f"Service TCP {self.name}: Socket set to listening mode",
            )
        ...
        while True:
            ...
            if __debug__:
                log(
                    "service",
                    f"Service TCP {self.name}: Inbound connection received "
                    f"from {cs.remote_ip_address}, port {cs.remote_port}",
                )
            threading.Thread(
                target=self.__thread_connection, args=(cs,)
            ).start()
    ...
    def service(self, cs: Socket) -> None:
        ...
        if __debug__:
            log(
                "service",
                f"Service TCP {self.name}: No service method defined, "
                "closing connection",
            )
        cs.close()
