...
class UdpParser:
    ...
    def __init__(self, packet_rx: PacketRx) -> None:
        ...
        if not packet_rx.parse_failed:
            packet_rx.frame = packet_rx.frame[udp.ps.UDP_HEADER_LEN :]
    ...
    def data(self) -> memoryview:
        ...
        if "_cache__data" not in self.__dict__:
            self._cache__data = self._frame[udp.ps.UDP_HEADER_LEN : self.plen]
    ...
    def header_copy(self) -> bytes:
        ...
        if "_cache__header_copy" not in self.__dict__:
            self._cache__header_copy = bytes(self._frame[: udp.ps.UDP_HEADER_LEN])
    ...
    def data_copy(self) -> bytes:
        ...
        if "_cache__data_copy" not in self.__dict__:
            self._cache__data_copy = bytes(self._frame[udp.ps.UDP_HEADER_LEN : self.plen - udp.ps.UDP_HEADER_LEN])
