...
class TcpSocket:
    ...
    def __init__(self, tcp_session=None):
        ...
        if tcp_session:
            ...
            self.local_ipv4_address = tcp_session.local_ipv4_address
            self.local_port = tcp_session.local_port
            self.remote_ipv4_address = tcp_session.remote_ipv4_address
    ...
    def __pick_random_local_port(self):
        ...
        used_ports = {int(_.split("/")[2]) for _ in stack.tcp_sessions if _.split("/")[1] in {"0.0.0.0", self.local_ipv4_address}}
    ...
    def connect(self, remote_ipv4_address, remote_port):
        ...
        self.tcp_session = tcp_session
        stack.tcp_sessions[tcp_session.tcp_session_id] = tcp_session
        self.logger.debug(f"{self.socket_id} -  Socket attempting connection to {remote_ipv4_address}:{remote_port}")
