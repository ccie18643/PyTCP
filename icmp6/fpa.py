...
class Icmp6Assembler:
    ...
    def __init__(
        ...
        mlr2_multicast_address_record: Optional[list[MulticastAddressRecord]] = None,
        ...
        self._mlr2_multicast_address_record: list[MulticastAddressRecord]
        self._mlr2_number_of_multicast_address_records: int

        if self._type == icmp6.ps.ICMP6_UNREACHABLE:
            self._un_reserved = 0
            self._un_data = b"" if un_data is None else un_data[:520]

        elif self._type == icmp6.ps.ICMP6_ECHOR_REQUEST:
        ...
        elif self._type == icmp6.ps.ICMP6_ECHOR_REPLY:
            self._ec_id = 0 if ec_id is None else ec_id
            self._ec_seq = 0 if ec_seq is None else ec_seq
            self._ec_data = b"" if ec_data is None else ec_data

        elif self._type == icmp6.ps.ICMP6_ROUTER_SOLICITATION:
            self._rs_reserved = 0

        elif self._type == icmp6.ps.ICMP6_ROUTER_ADVERTISEMENT:
    ...
    def __len__(self) -> int:
        ...
        if self._type == icmp6.ps.ICMP6_UNREACHABLE:
            return icmp6.ps.ICMP6_UNREACHABLE_LEN + len(self._un_data)

        if self._type == icmp6.ps.ICMP6_ECHOR_REQUEST:
            return icmp6.ps.ICMP6_ECHOR_REQUEST_LEN + len(self._ec_data)

        if self._type == icmp6.ps.ICMP6_ECHOR_REPLY:
            return icmp6.ps.ICMP6_ECHOR_REPLY_LEN + len(self._ec_data)

        if self._type == icmp6.ps.ICMP6_ROUTER_SOLICITATION:
            assert self._nd_options is not None
            return icmp6.ps.ICMP6_ROUTER_SOLICITATION_LEN + sum([len(_) for _ in self._nd_options])

        if self._type == icmp6.ps.ICMP6_ROUTER_ADVERTISEMENT:
            assert self._nd_options is not None
            return icmp6.ps.ICMP6_ROUTER_ADVERTISEMENT_LEN + sum([len(_) for _ in self._nd_options])

        if self._type == icmp6.ps.ICMP6_NEIGHBOR_SOLICITATION:
            assert self._nd_options is not None
            return icmp6.ps.ICMP6_NEIGHBOR_SOLICITATION_LEN + sum([len(_) for _ in self._nd_options])

        if self._type == icmp6.ps.ICMP6_NEIGHBOR_ADVERTISEMENT:
            assert self._nd_options is not None
            return icmp6.ps.ICMP6_NEIGHBOR_ADVERTISEMENT_LEN + sum([len(_) for _ in self._nd_options])

        if self._type == icmp6.ps.ICMP6_MLD2_REPORT:
            return icmp6.ps.ICMP6_MLD2_REPORT_LEN + sum([len(_) for _ in self._mlr2_multicast_address_record])
...
class Icmp6NdOptSLLA:
    ...
    def __len__(self) -> int:
        ...
        return icmp6.ps.ICMP6_ND_OPT_SLLA_LEN

    @property
    def raw_option(self) -> bytes:
        return struct.pack("! BB 6s", icmp6.ps.ICMP6_ND_OPT_SLLA, icmp6.ps.ICMP6_ND_OPT_SLLA_LEN >> 3, bytes(self._slla))
...
class Icmp6NdOptTLLA:
    ...
    def __len__(self) -> int:
        ...
        return icmp6.ps.ICMP6_ND_OPT_TLLA_LEN

    @property
    def raw_option(self) -> bytes:
        return struct.pack("! BB 6s", icmp6.ps.ICMP6_ND_OPT_TLLA, icmp6.ps.ICMP6_ND_OPT_TLLA_LEN >> 3, bytes(self._tlla))
...
class Icmp6NdOptPI:
    ...
    def __init__(
        ...
        self._code = icmp6.ps.ICMP6_ND_OPT_PI
        self._len = icmp6.ps.ICMP6_ND_OPT_PI_LEN
        self._flag_l = flag_l
        self._flag_a = flag_a
        self._flag_r = flag_r
    ...
    def __len__(self) -> int:
        ...
        return icmp6.ps.ICMP6_ND_OPT_PI_LEN
