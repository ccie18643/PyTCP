...
class Icmp6Parser:
    ...
    def ns_target_address(self) -> Ip6Address:
        ...
        if "_cache__ns_target_address" not in self.__dict__:
            ...
            self._cache__ns_target_address = Ip6Address(self._frame[self._hptr + 8 : self._hptr + 24])
    ...
    def na_target_address(self) -> Ip6Address:
        ...
        if "_cache__na_target_address" not in self.__dict__:
            assert self.type == icmp6.ps.ICMP6_NEIGHBOR_ADVERTISEMENT
            self._cache__na_target_address = Ip6Address(self._frame[self._hptr + 8 : self._hptr + 24])
    ...
    def nd_opt_slla(self) -> Optional[str]:
        ...
        if "_cache__nd_opt_slla" not in self.__dict__:
            ...
            for option in self.nd_options:
                if option.code == icmp6.ps.ICMP6_ND_OPT_SLLA:
    ...
    def nd_opt_tlla(self) -> Optional[str]:
        ...
        if "_cache__nd_opt_tlla" not in self.__dict__:
            ...
            for option in self.nd_options:
                if option.code == icmp6.ps.ICMP6_ND_OPT_TLLA:
    ...
    def nd_opt_pi(self) -> list:
        ...
        if "_cache__nd_opt_pi" not in self.__dict__:
            ...
            self._cache__nd_opt_pi = [_.prefix for _ in self.nd_options if _.code == icmp6.ps.ICMP6_ND_OPT_PI and _.flag_a and len(_.prefix.mask) == 64]
...
class Icmp6NdOptSLLA(icmp6.ps.Icmp6NdOptSLLA):
    ...
    def __init__(self, frame: bytes, optr: int) -> None:
        ...
        self.slla = MacAddress(frame[optr + 2 : optr + 8])
...
class Icmp6NdOptTLLA(icmp6.ps.Icmp6NdOptTLLA):
    ...
    def __init__(self, frame: bytes, optr: int) -> None:
        self.code = frame[optr + 0]
        self.len = frame[optr + 1] << 3
        self.tlla = MacAddress(frame[optr + 2 : optr + 8])
...
class Icmp6NdOptPI(icmp6.ps.Icmp6NdOptPI):
    ...
    def __init__(self, frame: bytes, optr: int) -> None:
        ...
        self.prefix = Ip6Network((Ip6Address(frame[optr + 16 : optr + 32]), Ip6Mask(f"/{frame[optr + 2]}")))


class Icmp6NdOptUnk(icmp6.ps.Icmp6NdOptUnk):
