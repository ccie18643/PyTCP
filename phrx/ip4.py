...
def _defragment_ip4_packet(self, packet_rx):
    ...
    header[0] = 0x45
    struct.pack_into("!H", header, 2, fpp.ip4.IP4_HEADER_LEN + len(data))
    header[6] = header[7] = header[10] = header[11] = 0
    struct.pack_into("!H", header, 10, inet_cksum(header, 0, fpp.ip4.IP4_HEADER_LEN))
    packet_rx = PacketRx(bytes(header) + data)
    fpp.ip4.Ip4Packet(packet_rx)
    if __debug__:
...
def _phrx_ip4(self, packet_rx):
    ...
    if packet_rx.ip4.proto == fpp.ip4.IP4_PROTO_ICMP4:
        self._phrx_icmp4(packet_rx)
        return

    if packet_rx.ip4.proto == fpp.ip4.IP4_PROTO_UDP:
        self._phrx_udp(packet_rx)
        return

    if packet_rx.ip4.proto == fpp.ip4.IP4_PROTO_TCP:
